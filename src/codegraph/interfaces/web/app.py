...
from codegraph.storage.queries import (
    class_hierarchy,
    file_impact,
    function_impact,
    graph_snapshot,
...
app = FastAPI(
    title="Codegraph",
    description="Code dependency graph, architectural risk scoring, ML similarity, and MCP server",
    version="1.0.0",
)
...
@app.get("/api/graph")
def api_graph(limit: int = Query(default=300, le=500)):
    driver = get_driver()
    try:
